...
export function useRoleRelationGraph() {
  ...
  const pushNode = (data: NodeData) => {
    if (nodes.value.some((node) => node.id === data.id)) {
      return;
    }
    nodes.value.push({
      id: data.id,
  ...
  const pushEdge = ({ source, target }: { source: string; target: string }) => {
    ...
    edges.value.push({
      ...
      markerEnd: {
        ...
        height: 20,
        color:
          "color-mix(in oklch, var(--colors-blue-500) var(--un-bg-opacity), transparent) /* oklch(0.623 0.214 259.815) */",
      },
      style: {
        stroke:
          "color-mix(in oklch, var(--colors-blue-500) var(--un-bg-opacity), transparent) /* oklch(0.623 0.214 259.815) */",
  ...
  const fetch = (roleId: string) => {
    return new Promise((resolve) => {
      getRoleInfo(roleId).then((roleInfo) => {
        ...
        nextTick(() => {
          ...
          instance.fitView().then(() => resolve(true));
        });
      });
    });
  };
