...
<template>
  <div class="flex flex-col items-center">
    ...
    <div class="text-zinc-50 px-10 py-2 text-center rounded bg-blue-500 min-w-16">
      <component :is="data.label" v-if="isVNode(data.label)" />
      <span v-else>{{ data.label }}</span>
    </div>
    <handle type="source" class="opacity-0" :position="Position.Bottom" />
    <div v-if="data.showExpand" class="i-material-symbols:add color-blue-500 cursor-pointer" @click="emits('expand', data.id)" />
  </div>
</template>
