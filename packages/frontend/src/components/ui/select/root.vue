...
const defaultDisplayBehavior = (
  val: T extends SelectProps<infer R> ? R : unknown,
) => {
  return val;
};
...
const {
  options,
  multiple,
  displayBehavior = defaultDisplayBehavior,
} = toRefs(props);

const renderOptions: Ref<OptionProps<AcceptableValue>[]> = ref<
  OptionProps<AcceptableValue>[]
>([...(options.value ?? [])]);
...
<template>
  ...
  >
        ...
        <slot name="trigger">
          ...
          >
            <div
              ...
              :key="idx"
              class="text-white px-2 py-1 rounded bg-blue-500 flex gap-0.5 h-full w-fit items-center"
            >
              <span class="text-sm">{{ displayBehavior?.(tag as any) }}</span>
              <div
                class="i-material-symbols:close"
                @click.stop="onRemove(tag)"
              />
            </div>
            <span
    ...
    <combobox-content
      class="h-[200px] w-[var(--reka-combobox-trigger-width)] z-10"
      position="popper"
      :side-offset="16"
    >
      <div
