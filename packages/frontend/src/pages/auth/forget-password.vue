...
const ttl = ref(-1);
const sendMailCode = () => {
  axios.post<never, { ttl: number }>('/secure/password/forget-mail-code', null, { params: { email: forgetPassword.email } })
    .then((resp) => {
          ...
          <tiny-form-item label="验证码" required prop="code">
            <div class="flex gap-2 w-full">
              ...
              <count-down v-model="ttl" :interval="1000" :end-value="0">
                <template #done>
                  <tiny-button :disabled="!forgetPassword.email.length" @click="sendMailCode">
                    发送验证码
                  </tiny-button>
                </template>
              </count-down>
